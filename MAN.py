...
parse.add_option('-a', dest='Algorithm',action='store',type=str,default='all',help='Select the algorithm for calculating the correlation coefficient.[1-81],(DEFAULT=0)' )
...
def main():
    ...
    p_valves = get_pvalue(matrix, group1, group2)
    save_p_result(matrix,p_valves,outdir+'DFG_result.csv')#Save the DFG result
    #step2:
    matrix1 = np.delete(matrix, 1, 1)
    trim_row = np.where(p_valves[:, -1] < 0.05)[0] + 1
    ...
    saveNetData(outdata,outdir+'Netwrok_result.csv')# Save the network result
...
def get_pan_matrix(pan_result):
    f = open(pan_result, 'r')
    f_csv = csv.reader(f)
    lines = [line for line in f_csv]
    matrix = np.array(lines, dtype=object)
    head = matrix[0]
    trimed_index = [0,2]+list(range(options.NUM,len(head)))
