...
def main():
    ...
    save_p_result(matrix,p_valves,outdir+'Statistical_test__result.csv')#Save the DFG result
    #step2:
    ...
    if options.Algorithm ==0:
        for n in range(1,82):
            method = eval('methods.method_%s'%n)
            outdata = create_net(pan_dic, method,sample_len)
            saveNetData(outdata,outdir+'Netwrok_result_method%s.csv'%n)# Save the network result
    else:
